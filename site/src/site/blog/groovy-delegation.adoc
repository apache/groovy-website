...
transform. Here we annotate two properties (our delegates).
...
added to our classes implements clause.
...
every public method in the two classes (about 120 methods in total).
Also, any interfaces implemented by our delegates are also automatically
added to our `Menu` class's implements clause.
...
If we don't really need all the list and date methods implemented,
we can just delegate to the ones we are interested in. In this case,
we also need to disable the automatic collection of delegate interfaces.
...
but doesn't implement the `List` interface.
