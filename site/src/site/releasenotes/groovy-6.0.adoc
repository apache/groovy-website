...
We are currently finalising Groovy 6.0. Caution should be exercised
if using incubating features as the details may change before final release.
We are also making changes based on feedback - please get your feedback in as early as possible.
Apologies for those trying these early versions if certain aspects change before the final release.

NOTE for RC-1: RC-1 defaults to turning on a reflective cold dispatch tier.
This is a setting currently needed for native images like GraalVM but changes
what stack frames would be visible to logging libraries. This tier will be
opt-in only for future releases. Users outside native scenarios may want to set
the flag to opt-out for RC-1, or use the other workarounds mentioned like @CompileStatic.
See https://issues.apache.org/jira/browse/GROOVY-12354[GROOVY-12354] for more details.
|===
...
- <<channel-select,`ChannelSelect`>> covers the full CSP choice: mixed send/receive offers, `when` guards, `fair()`/`random()` policies, and timer branches — with losing branches left untouched.
...
- *Reflection-free MOP dispatch*: once a method has been called often enough, `CachedMethod.invoke` installs a generated hidden nestmate that `INVOKE`s the target directly instead of going through `Method.invoke`, so the JIT sees a constant target where it previously saw a reflective call (https://issues.apache.org/jira/browse/GROOVY-12325[GROOVY-12325]).
...
- <<error-format,`groovyc --error-format short`>> emits one `file:line:column: severity: message` line per diagnostic — the shape editors, CI log parsers and coding agents already understand.
- <<syntax-errors,Clearer syntax error messages>> for the mistakes people actually make: unclosed strings and comments reported where they opened, an illegal escape in a Windows path named as one, invisible pasted Unicode shown as `+\u200b+` rather than as nothing at all, missing punctuation named (`Missing '('`), and reserved words answered with the Groovy spelling (`'const' is not supported; use 'val' or 'static final' instead`).
...
- <<java-generics,Generic type syntax>> now follows the JLS: rare types, nested parameterizations, `instanceof` reifiability and type-variable scoping all behave as `javac` does, with matching diagnostics.
- <<switch-alignment,`switch` follows Java's rule>> that the arrow decides fall-through and the position decides whether a value is produced -- so an arrow `switch` used as a statement no longer has to be exhaustive. Statically compiled switch _statements_ also gain `tableswitch`/`lookupswitch` dispatch and the duplicate-label diagnostic that expressions already had.
...
- <<non-hotspot,Non-HotSpot runtimes>>: the runtime no longer assumes it is running on a HotSpot JVM built from a current JDK — version detection, accessibility rules, hidden classes, `ScopedValue` and the indy exception handlers all degrade instead of failing, which is what it takes to come up on Android's ART.
...
- Path containment and least privilege across the tooling: GroovyDoc writes only inside its destination directory and never follows a symbolic link out of `doc-files/`, `grape uninstall` deletes only inside the module cache, `deleteDir` treats a Windows junction as a leaf instead of entering it, the `groovy.ast=xml` dump inherits the source file's permissions rather than the umask, and a failing groovlet no longer returns internals to the client.
...
select now works by _claim_ -- the branches share one flag that a
...
(https://issues.apache.org/jira/browse/GROOVY-12320[GROOVY-12320]).

image:img/groovy6_select.png["Mixed send and receive offers, per-branch guards and timer branches, with a choice policy you pick",width=800]
...
taken within _n_ calls (JCSP's `fairSelect`), and `random()` shuffles
it so every ready channel is equally likely (Go's policy). Keep the
instance across calls for `fair()` to rotate.

*Mixed offers.* A branch may _offer to send_ as well as receive -- "I
will send my opener, or take my peer's if it sends first"
(https://issues.apache.org/jira/browse/GROOVY-12323[GROOVY-12323]):
...
(https://issues.apache.org/jira/browse/GROOVY-12326[GROOVY-12326]):
...
position, so `result.index` denotes the same branch either way --
dropping the offer from the argument list instead would silently
...
computed as a whole
(https://issues.apache.org/jira/browse/GROOVY-12324[GROOVY-12324]);
...
(https://issues.apache.org/jira/browse/GROOVY-12343[GROOVY-12343]).
The timer _offer_ `after` re-arms on every round; the timer _channel_
...
See the dochome6:core-async-await.html[Async/Await user guide] for the
...
| `ChannelSelect` claim protocol, `fair()`, `random()`
| A select takes a value from exactly one channel and leaves the losing
branches untouched, rather than consuming and re-sending. `fair()`
rotates the scan (every ready channel taken within _n_ calls);
`random()` chooses uniformly; the default remains priority by list
order. See <<channel-select>>.
| https://issues.apache.org/jira/browse/GROOVY-12320[GROOVY-12320]

| `ChannelSelect` mixed offers
| `offers(send(ch, v), receive(other))` -- a branch may offer to send as
well as receive. Exactly one offer commits; a retired send leaves no
buffered residue and no waiting sender.
| https://issues.apache.org/jira/browse/GROOVY-12323[GROOVY-12323]

| `ChannelSelect` preconditions
| `receive(ch).when { ... }` guards a branch, and `select(flag, flag)`
supplies the same guards positionally. A guarded-off branch is not
registered but keeps its index; enabling none fails with
`IllegalStateException`. `Result.getChannel()` names the winning channel.
| https://issues.apache.org/jira/browse/GROOVY-12324[GROOVY-12324],
https://issues.apache.org/jira/browse/GROOVY-12326[GROOVY-12326]

| Timer branches
| `after(millis)` is a timer _offer_ re-armed each round;
`AsyncChannel.after(millis)` is a timer _channel_ holding a fixed
deadline. Both accept a `Duration`, take part in the claim protocol,
and compose with `when`, `fair()` and `random()`. `after(0)` listed
last is the `default` clause of Go's `select`.
| https://issues.apache.org/jira/browse/GROOVY-12343[GROOVY-12343]
...
every JDK
(https://issues.apache.org/jira/browse/GROOVY-12274[GROOVY-12274]).
...
(https://issues.apache.org/jira/browse/GROOVY-12351[GROOVY-12351]).
...
=== A closed, citable grammar

The user guide gains a new *Grammar* chapter: a complete EBNF projection
of `src/antlr/GroovyLexer.g4` and `src/antlr/GroovyParser.g4`, covering
every parser rule, lexer token, fragment and mode recognised at
compilation phase `PARSING`. It is written for readers who need a closed
grammar to cite rather than examples to follow -- tool authors,
implementers, language researchers, and the AI assistants increasingly
asked "is this valid Groovy?"

Groovy's surface syntax is not a pure context-free grammar, and the
chapter says so explicitly, specifying it in three layers: the lexical
context machine (lexer modes, grouping stack, newline hiding, token-type
rewrites), a CFG skeleton in EBNF, and the semantic predicates and
restrictions that constrain it. It also documents where the EBNF
over-approximates the real language, what is intentionally excluded, and
a production index. The `.g4` files remain the implementation source of
truth: where a production and a rule disagree, the rule wins until the
chapter is updated.
...
[[switch-alignment]]
== Switch: Position Decides, Not the Arrow

Groovy 4 introduced Java's arrow form for `switch`, and Groovy 6 compiles
switch expressions as first-class AST (see
<<Groovy6.0-breaking,Breaking changes>>,
https://issues.apache.org/jira/browse/GROOVY-12255[GROOVY-12255]).
Groovy 6 completes that work by adopting Java's rule (JEP 361) for which
of the two a given `switch` is: *the arrow decides fall-through; the
position decides whether a value is produced.*

That rule is also the foundation the draft
link:../wiki/GEP-19.html#switch-position[GEP-19: Structural Pattern
Matching in `switch`] builds on -- pattern matching inherits the position
rule rather than introducing one of its own -- though pattern matching
itself is not part of 6.0.

=== A switch whose value is unused is a statement

A `switch` in statement position is a statement whatever its arms look
like (https://issues.apache.org/jira/browse/GROOVY-12399[GROOVY-12399],
https://issues.apache.org/jira/browse/GROOVY-12408[GROOVY-12408]).
Nothing is thrown when no arm matches, and no `default` is required:
...
switch (i) {
    case 1 -> r = 'one'
}                       // i == 2: nothing happens, as the colon form always did
----

A `switch` that ends a method, closure or script body is a statement too,
and Groovy's implicit return does to it what it does to any other
statement -- so the value of the matched arm still comes back:
...
assert describe(3) == null      // unmatched: null, not an exception
----

A primitive-returning method whose bare `switch` matches nothing yields
the type's default, exactly as `int f(int i) { if (i == 1) return 1 }`
and the colon form always have.

Expression positions -- a declaration, a `return`, an argument, the arm
of an enclosing switch expression -- keep the strict rules: an unmatched
selector throws `IllegalStateException`, and under `@TypeChecked` /
`@CompileStatic` a non-exhaustive switch expression is a compile-time
error:

[source,groovy]
----
def s = switch (i) { case 1 -> 'one' }   // IllegalStateException when unmatched
----

The same question is asked of a nested `switch`: a `switch` written as
the sole expression of an arrow arm produces a value only when the
`switch` owning that arm does, so a statement switch does not force
strictness on the switches inside it.

=== Jump-table dispatch for statically compiled switch statements

A statically compiled switch _expression_ already emitted `tableswitch` /
`lookupswitch`; a switch _statement_ never had, in any Groovy release, so
the colon form compiled to a chain of `isCase` calls linear in the number
of arms. Statements now get the same dispatch
(https://issues.apache.org/jira/browse/GROOVY-12405[GROOVY-12405]):
...
// was: three ScriptBytecodeAdapter.isCase calls; now: one tableswitch
----

The jump table is emitted only where the inferred selector type and every
label agree and the labels are constants `javac` would switch on --
int-family, `String` and enum constants. Anything else keeps the
sequential chain. Statement semantics are preserved: arms are laid out in
source order so one that does not jump falls into the next, `break` /
`continue` / `return` / `throw` compile as before, and a null selector
takes the default.

=== Duplicate case labels are reported

https://issues.apache.org/jira/browse/GROOVY-12289[GROOVY-12289] rejected
a repeated constant case label in a switch expression but left statements
out. Java rejects a duplicate in either position, and so does Groovy 6 now
(https://issues.apache.org/jira/browse/GROOVY-12406[GROOVY-12406]):
...
def m(int x) {
    switch (x) {
        case 1: return 'a'
        case 1: return 'b'   // dead: the first arm always wins -- now a compile error
        default: return 'c'
    }
}
----

Only labels whose value is statically known are compared -- the same set
the jump-table optimizers key on. A label the compiler cannot evaluate,
such as a variable or a call, is left to first-match-wins dispatch. This
is a type-checking diagnostic: dynamic Groovy is unchanged and still
takes the first matching arm.

=== Case labels resolve the same way in either position

Under `@CompileStatic` a switch expression called the `isCase` method the
type checker selected, while a switch statement dispatched on the label's
runtime type, so the same label could give two different answers. Both
positions now use the target the type checker selects
(https://issues.apache.org/jira/browse/GROOVY-12407[GROOVY-12407]):
...
switch (x) { case label -> r = 'y'; default -> r = 'n' }  // was 'y'
return switch (x) { case label -> 'y'; default -> 'n' }   // was 'n'
----

A label with no applicable `isCase` stays a compile error in a switch
expression but keeps the dynamic call in a switch statement, so code that
has always compiled still does. `case null` likewise keeps the dynamic
comparison. Dynamic Groovy is untouched, and jump-table dispatch still
...
On the untyped path, `YamlSlurper` and `TomlSlurper` now offer the same
`setDateHandling(JsonDateHandling)` choice `JsonSlurper` has, so a
date-like string can become a `Date`, an `Instant` or an
`OffsetDateTime` rather than only a `String`. The default is unchanged --
`STRING`, which is what these slurpers have always produced -- and a
date carrying no time (`"2026-09-04"`) stays a `String` whatever is
chosen. Choosing anything other than `STRING` switches the underlying
parser to `INDEX_OVERLAY` so the conversion takes effect, which means a
map with more than a handful of entries no longer iterates in document
order
(https://issues.apache.org/jira/browse/GROOVY-12357[GROOVY-12357]).
====

NOTE: `YamlSlurper` does not resolve YAML anchors and aliases -- an alias
...
(https://issues.apache.org/jira/browse/GROOVY-12358[GROOVY-12358]).
...
`StreamingSAXBuilder` called neither validator, so a comment body, an
instruction target or non-map instruction data carrying `-->` or `?>`
closed the construct early and whatever followed was emitted as markup;
it now applies the same checks. This rejects input that was previously
accepted but was never well-formed XML -- see
<<Groovy6.0-breaking,Breaking changes>>.
| https://issues.apache.org/jira/browse/GROOVY-12103[GROOVY-12103],
https://issues.apache.org/jira/browse/GROOVY-12338[GROOVY-12338]
|===
...
* `grape uninstall` no longer deletes outside the module cache. The jar to
delete was `new File(jardir, name)` with `name` taken from the module's
cached Ivy descriptor unchecked, so a descriptor whose artifact name carried
a path separator or a `..` segment deleted a file above the module's jars
directory. Deletion is now skipped, with a warning, unless the resolved file
is inside that directory, checked on canonical paths so a `..` or a symbolic
link along the way cannot carry it out
(https://issues.apache.org/jira/browse/GROOVY-12369[GROOVY-12369]).
...
gapi:groovy.util.logging.Log4j2[`@Log4j2`] gains a `staticLocation`
option that does just that
(https://issues.apache.org/jira/browse/GROOVY-12378[GROOVY-12378]):
...
<<g6-other-core-api,Other Core API Changes>>.
...
example -- no longer need to copy `Main`.
`Main.start(GroovyshOptions, String[])` is a programmatic entry point that does not call `System.exit`, and
`GroovyshOptions.builder()` carries the supported hooks
(https://issues.apache.org/jira/browse/GROOVY-12403[GROOVY-12403]). The
API is incubating, and mirrors JLine's `ShellBuilder` name for name
wherever an equivalent exists, so that a later move onto
`org.jline.shell` need not break it:
...
int rc = Main.start(GroovyshOptions.builder()
    ...
    .prompt { 'gremlin> ' }
    .onReaderReady { reader, registry ->           // key bindings, completion
    ...
    .groups(new SimpleCommandGroup('MyApp', new AbstractCommand('/ping') {
        @Override Object execute(CommandSession session, String[] args) { 'pong' }
    }))
    .resultHandler { printer, result -> ... }      // render values your way
    .errorHandler { error, defaultTrace -> defaultTrace.accept(error) }
    .build(), '-e', 'answer')
----

Command groups are resolved _before_ groovysh's own, so a host may
override a built-in command name, and the `onReaderReady` callback fires
once the reader and the registry are both wired -- which is what makes
whole-line completion of a host's own domain language reachable, rather
than only the arguments of its commands. Compile-time customizers go on
the engine: `new GroovyEngine(config)` takes a `CompilerConfiguration`,
and `GroovyEngine.getClassLoader()` is the public accessor for the engine
class loader. Binding maps stay what they always were -- variables
visible to evaluated scripts -- and are not the place for embedding
hooks. The published API no longer names JLine types deprecated since
JLine 4.0, and the POSIX-style console commands move out of `Main` into a
reusable `ExtraConsoleCommands` registry, each with its own `/help`
description. The groovysh chapter of the user guide documents each hook.
...
path resolves within `destdir`. The old check rejected only absolute
names, so a package path picking up a `..` segment from the source path
could write a page above the destination directory
(https://issues.apache.org/jira/browse/GROOVY-12360[GROOVY-12360])

| `doc-files/` copies do not follow links
| The verbatim mirror of a package's `doc-files/` and `snippet-files/`
used `Files.copy`, which follows a symbolic link and copies the content
of its target -- so a link pointing outside the source tree published
that outside content. A link, or other reparse point such as a Windows
junction, is now skipped with a warning; destination paths are also
normalised to `/` so a copied asset is not stored under a backslash key
on Windows
(https://issues.apache.org/jira/browse/GROOVY-12370[GROOVY-12370])
...
Groovy 6 closes the remaining gaps between Groovy's and Java's handling
of generic type syntax
(https://issues.apache.org/jira/browse/GROOVY-12319[GROOVY-12319]), so
generic declarations copied from Java source mean the same thing in
Groovy. Three strands:
...
_names_ rather than parameterizations (JLS 15.8.2), and method and
...
<<Groovy6.0-breaking,Breaking changes>>, and the _Generics_ section of
...
* *Reachability metadata ships in the jars*: building an image of a
dynamic Groovy program used to require a native-image agent run just to
capture what Groovy's own runtime reflects over; without it the image
built and then failed initialising `GroovySystem`. The groovy jar now
carries that metadata under `META-INF/native-image/org.apache.groovy/groovy/`,
where `native-image` and the GraalVM build plugins find it: a
`native-image.properties` pinning run-time initialisation for the
runtime classes that decide how dispatch links
(https://issues.apache.org/jira/browse/GROOVY-12366[GROOVY-12366]),
the resources the runtime reads, and a `reachability-metadata.json`
that the build generates from what the runtime is known to do rather
than from a recording: the metaclass registry's bootstrap, the method
handles the invokedynamic machinery obtains, the Groovy-owned types
that get a metaclass, and the `BeanInfo`/`Customizer` probes the
runtime expects not to find. Every entry is conditional on the class
performing the access being reached, so an image pays only for what it
uses. The default Groovy method adapters (`dgm$N`) are now constructed
directly through a generated factory instead of being loaded by name,
so all of them are reachable without metadata; `-Dgroovy.dgm.factory=false`
passed to `native-image` restores name-based loading and keeps only the
adapters an agent recording saw, for a smaller image. Modules that
register extension methods (`groovy-nio`, `groovy-xml`, `groovy-sql`,
`groovy-swing` and others) ship their own metadata alongside. The agent
is still the way to record what depends on the application, its own
classes and the JDK types it calls dynamically; run it with
`-Dgroovy.indy.aot.link=true` so the JVM links call sites as the image
will. A native probe under `subprojects/tests-native` and a CI check
keep the shipped metadata in step with the runtime. GraalVM for JDK
25.0.4 or later is required; earlier releases carry a substitution
targeting a method Groovy 6 no longer has
(https://issues.apache.org/jira/browse/GROOVY-12365[GROOVY-12365]).
* *The module metadata covers more than extension registration*:
`groovy-nio` ships the `WatchEvent$Modifier` entry that `Path` introspection
needs, which a JVM agent run cannot record and so has to be hand-listed
(https://issues.apache.org/jira/browse/GROOVY-12394[GROOVY-12394]), and
`groovy-concurrent-java` -- which repackages the async runtime classes
without the groovy jar, and so without the groovy jar's metadata -- carries
its own copy of the entries registering the virtual-thread executor and the
`ScopedValue` lookups, so a pure-Java image gets them too
(https://issues.apache.org/jira/browse/GROOVY-12380[GROOVY-12380]). The
native probe builds its image over `groovy-nio`, `groovy-xml` and
`groovy-json` alongside core, which is what surfaced `groovy-xml`'s JAXP
`ServiceLoader` lookups and `groovy-json`'s service-loaded string service as
gaps in the first place
(https://issues.apache.org/jira/browse/GROOVY-12379[GROOVY-12379]).
* *Image frames are recognised as runtime frames*: inside a native image
GraalVM runs dynamically built `MethodHandle` chains in its own interpreter
and reflects through its own accessors, so `com.oracle.svm.*` frames sit
between a Groovy call and its real caller.
`ReflectionUtils.getCallingClass` -- which is how `getBundle` and Grape find
the calling class -- and `StackTraceUtils.sanitize` now treat those as
runtime frames, so a resource bundle resolves against the caller that asked
for it and a sanitized stack trace is not padded with image internals
(https://issues.apache.org/jira/browse/GROOVY-12362[GROOVY-12362]).
...
listed in `jdk.logger.packages` _at image build time_ (a run-time `-D` is
...
(https://issues.apache.org/jira/browse/GROOVY-12354[GROOVY-12354]).
...
(https://issues.apache.org/jira/browse/GROOVY-12381[GROOVY-12381]).

[[non-hotspot]]
== Running on Non-HotSpot Runtimes

The work on native images turned up a second, related assumption: Groovy's
runtime had grown a handful of dependencies on being a HotSpot JVM built from
a current JDK. Those matter for any runtime that implements the Java class
library without being a JDK -- Android's ART most of all, which is where most
of the following surfaced. Groovy 6 removes them, so the runtime comes up and
dispatches dynamically on such a runtime rather than failing during class
initialisation.

None of this makes Groovy 6 a fully-supported, tested Android platform.
What changed is that these are no longer in the way. While you can build
Android apps using Groovy 6 with a shim architecture,
we anticipate full support in a future Groovy version.
...
| `Runtime.version()` is a Java 9 API that a runtime built on the JDK class
library need not provide. The new `JavaFeatureVersion.current()` parses
`java.specification.version` where it is missing, and assumes the lowest
release Groovy supports when that is unusable; `VMPluginFactory` and
`CompilerConfiguration`'s default `targetBytecode` go through it
(https://issues.apache.org/jira/browse/GROOVY-12382[GROOVY-12382])

| Accessibility without a module system
| The VM plugin's accessibility rules consulted `Module`/`Layer`. Where the
module system is absent, they now fall back to the Java 8 rules instead of
failing
(https://issues.apache.org/jira/browse/GROOVY-12383[GROOVY-12383])

| Type-use annotations without `AnnotatedType`
| The Java 8 plugin reads type-use annotations through `AnnotatedType`, which
not every reflection implementation carries. It is now skipped rather than
assumed
(https://issues.apache.org/jira/browse/GROOVY-12389[GROOVY-12389])

| Hidden classes
| The direct invokers (see <<g6-other-core-api,Other Core API Changes>>)
check for Android *before* touching `HiddenClassDefiner`, so a runtime with
no hidden-class support never loads the definer at all
(https://issues.apache.org/jira/browse/GROOVY-12384[GROOVY-12384])

| `ScopedValue` detection
| `ScopedLocal` decided whether `ScopedValue` was available from the runtime
version; it now probes for the type
(https://issues.apache.org/jira/browse/GROOVY-12385[GROOVY-12385])

| `catchException` on ART
| ART does not run the handler in a `MethodHandles.catchException` combinator
for an exception constructed without a writable stack trace -- which is
exactly what Groovy's control-flow exceptions are, so `null.foo()` surfaced a
no-stack `MissingMethodException` instead of the `NullPointerException` from
`NullObject.invokeMethod`, and no-stack exceptions escaped unwrapped. The
`GroovyObject` fallback and the unwrapper are now available as plain Java
`try`/`catch` handlers, used only when running on Android; the JVM path is
untouched
(https://issues.apache.org/jira/browse/GROOVY-12387[GROOVY-12387])

| Lazy `System.Logger` in `groovy-json`
| `System.Logger` was obtained in static initialisers, so a runtime without a
usable `LoggerFinder` failed initialising classes that never log. It is now
obtained lazily
(https://issues.apache.org/jira/browse/GROOVY-12386[GROOVY-12386])

| Compiler types out of the runtime
| `GStringImpl`'s constant-value check reached into the compiler's
`ClassHelper` via `ImmutablePropertyUtils`, pulling a slice of the compiler
into any program that used a `GString`. The type knowledge moved to a
runtime-only `ImmutableTypes`
(https://issues.apache.org/jira/browse/GROOVY-12390[GROOVY-12390])

| DGM adapter initialisers
| The generated `dgm$N` adapters held `MethodType` constants in their static
initialisers, which a runtime with a narrower constant-pool implementation
cannot resolve; they are now built without them
(https://issues.apache.org/jira/browse/GROOVY-12388[GROOVY-12388])

| R8 keep rules
| `IndyCatchCompat` obtained its handles by name, which a shrinker cannot
see, so R8 needed a keep rule to avoid removing them. The lookups are now
direct references
(https://issues.apache.org/jira/browse/GROOVY-12395[GROOVY-12395])

| `grooid` jar repackaging
| The `grooid` build rewrites `java.beans.*` references to `openbeans`, and
was rewriting `java.beans.Transient` -- a type openbeans never had -- to a
class that does not exist. The rewrite now leaves it alone
(https://issues.apache.org/jira/browse/GROOVY-12391[GROOVY-12391])
|===
...
(https://issues.apache.org/jira/browse/GROOVY-12312[GROOVY-12312]).

image:img/groovy6_errorformat.png["The same diagnostics rendered two ways: the FULL default with source line and caret, and one parseable line per diagnostic under SHORT",width=800]
...
Together with `--check` (see <<g6-other-core-api,Other Core API
...
(https://issues.apache.org/jira/browse/GROOVY-12169[GROOVY-12169],
https://issues.apache.org/jira/browse/GROOVY-12171[GROOVY-12171]) and gives
...
(https://issues.apache.org/jira/browse/GROOVY-12353[GROOVY-12353]).

An unterminated string or comment is reported where it *opened*, rather than
at whatever the lexer eventually tripped over:
...
A *closed* literal carrying a backslash that is not a Groovy escape -- the
Windows path everyone writes at least once -- is the illegal escape it
actually is, not an unclosed quote, and the caret sits on the backslash:
...
A word that is reserved but not supported says so, and offers the Groovy
...
The remaining everyday parse failures are named the same way
(https://issues.apache.org/jira/browse/GROOVY-12401[GROOVY-12401]).
Array creation says which half is wrong -- `Array dimension missing;
specify a size or add an initializer '{}'`, `Cannot specify an array size
after an empty dimension`, `Cannot combine an array size with an array
initializer`. A `do` block reports `Missing 'while'`, or, when a `while`
does follow, the real problem: `do-while body must be a single statement;
wrap multiple statements in '{ }'`. Structural slips get their own
sentence too -- `';' expected after the last enum constant`, `'import' is
only allowed at the beginning of a compilation unit`, `'default' cannot
follow a method header; put 'default' before the method name`, and
`Method definition not expected here`. A closer or separator with nothing
to close is named rather than described as unexpected input: `Unexpected
')'`, `Unexpected ']'`, `Unexpected '}'`, `Unexpected ','`, `Unexpected
';'`.

`Missing '>'` is also now decided from the token _after_ the `<`, which
must look like a type argument. A comparison never has a closing `>`
either, so `if (MAX < x)` followed by an unrelated syntax error was
previously rewritten as an unclosed type-argument list; it no longer is.

These compose with the rest of the diagnostics work: `--error-format short`
(see <<error-format>>) renders each of them as one parseable line, and parser
...
`--check` stops after `Phases.CLASS_GENERATION` rather than before it, so
that the errors only class verification and bytecode generation can report
-- duplicate method signatures, illegal modifier combinations,
abstract/final conflicts, direct field access diagnostics -- are reported
too. Bytecode is built in memory and discarded; the `OUTPUT` phase that
would write the class files is what `--check` suppresses. Selecting an
earlier phase through `setTargetPhase` (or a compiler configuration script,
which is applied after the option and so overrides it) remains available
for a deliberately shallower check
(https://issues.apache.org/jira/browse/GROOVY-12311[GROOVY-12311]).
* *Error tolerance now covers type-checking errors*. The configured
tolerance (`groovyc -t`, `CompilerConfiguration.tolerance`) was only
enforced for errors reported through `SourceUnit.addError`, so it had no
effect on the most common error class in `@CompileStatic` code: errors
reported by visitors, including the static type checker, were unbounded
however low the tolerance was set. Both routes are now tolerance-aware
(the temporary collectors the type checker pushes for speculative checks
still collect without bailing out). A tolerance of zero is treated as
_unlimited_ rather than fail-fast, and the `<groovyc>` Ant task gains a
matching `tolerance` attribute for both its forked and in-process paths
(https://issues.apache.org/jira/browse/GROOVY-12306[GROOVY-12306]).
...
* *Reflection-free MOP dispatch*: once a `CachedMethod` has been invoked
often enough (`groovy.cachedmethod.invoker.threshold`, default 1000,
aligned with `groovy.indy.optimize.threshold`), `CachedMethod.invoke`
installs a generated hidden nestmate that `INVOKE`s the target directly
-- or `invokeExact`s a `classData` `MethodHandle` -- instead of calling
`Method.invoke`. The JIT then sees a constant target where it previously
saw a reflective call. This is the MOP path only: `invokedynamic` keeps
its call-site `Lookup` and is not fed the trampoline. Set
`-Dgroovy.cachedmethod.invoker.disable=true` to stay on `Method.invoke`
(https://issues.apache.org/jira/browse/GROOVY-12325[GROOVY-12325]).
* *Reflective cold dispatch tier off by default on the JVM*: the
experimental tier that dispatches a plain method call through
`Method.invoke` until its call site has been hit
`groovy.indy.optimize.threshold` times
(https://issues.apache.org/jira/browse/GROOVY-12137[GROOVY-12137]) is
now used only by AOT-linked call sites (GraalVM native images, where it
is the steady state, or `-Dgroovy.indy.aot.link=true`). While a site is
on the tier, reflection and Groovy runtime frames sit between the caller
and the target, so logging frameworks (`%C`/`%M`/`%L`, the JUL source
class and method), `StackWalker` and stack traces report the runtime as
the caller until the site promotes -- forever for a rarely hit `warn` or
`error`. Groovy 5 reported the caller correctly from the first call, so
on a JVM the tier is now opt-in via `-Dgroovy.indy.cold.reflection=true`.
_6.0.0-RC-1 shipped with the tier on by default_: RC-1 users can pass
`-Dgroovy.indy.cold.reflection=false` as a JVM argument to get the 5.x
behaviour back. Calls the runtime dispatches through the metaclass, such
as a dynamic method name (`log."$level"(msg)`) or an explicit
`invokeMethod`, show runtime frames on every Groovy version; the
invokedynamic guide documents the JUL and Logback settings that cover
those, on a JVM and in native images
(https://issues.apache.org/jira/browse/GROOVY-12354[GROOVY-12354]). Log4j2
has no equivalent setting, so `@Log4j2` gains a `staticLocation` option that
supplies the location from the compiler instead of leaving it to a stack
walk -- see <<platform-logging,Platform Logging>>.
* *`deleteDir` treats a junction as a leaf.* Since GROOVY-12125 a symbolic
link inside a tree being deleted is removed as the link rather than entered,
but the guard was `Files.isSymbolicLink`, which does not report a Windows
directory junction -- the link form that matters most there, since
`mklink /J` needs no privilege where a Windows symbolic link needs one most
users do not hold. `File.deleteDir` and the `groovy-nio` `Path.deleteDir`
now read each node's attributes without following links and treat anything
that is neither a regular file nor a directory as a leaf, so a junction or
other reparse point, a pipe, a socket or a device is removed as the node it
is and never entered. On POSIX the visible behaviour is unchanged
(https://issues.apache.org/jira/browse/GROOVY-12359[GROOVY-12359]).
* The `groovy.ast=xml` debug option writes a serialized AST to
`<source>.xml` beside the source. It was created at the process umask, so a
source the developer had kept private produced a world-readable dump of its
AST. The dump is now created with the source file's own permissions, and
owner-only where there is no source file to match -- created *with* the
permissions rather than created and then adjusted, so its contents are never
briefly readable by anyone those permissions exclude
(https://issues.apache.org/jira/browse/GROOVY-12368[GROOVY-12368]).
* *A bounded parser DFA cache*. The shared ANTLR DFA cache had no ceiling
out of the box, and the GC canary that dropped it is only observed on the
parse path, so a long-lived daemon parsing across many modules could grow
it unchecked between observations -- surfacing as time spent collecting
rather than parsing, or as an `OutOfMemoryError` at a tight heap.
`groovy.antlr4.cache.size` now bounds the DFA states retained across the
shared ATN, defaulting to 20000; exceeding it drops the cache. Bounding by
size costs nothing while the cache is small, so a project that never
reaches the limit keeps a fully warm cache; the older parse-count
`groovy.antlr4.cache.threshold` reverts to opt-in
(https://issues.apache.org/jira/browse/GROOVY-12318[GROOVY-12318]). The
threshold and the GC canary also compose rather than excluding each other:
setting a positive threshold previously switched the canary off, so
reaching for the documented knob to reduce memory use silently removed the
one mechanism that responds to actual memory pressure. A negative value
remains the explicit "never clear" escape hatch, now switching off both
(https://issues.apache.org/jira/browse/GROOVY-12316[GROOVY-12316]).
* *Field-backed property access is consistent across compilation modes.*
A non-public field of a strongly encapsulated class -- a JDK class without
`--add-opens`, say -- could be selected as a meta property and then fail
only on read or write, with the `invokedynamic` property path escalating
the refusal to `GroovyBugError` and attribute access leaking a raw
`IllegalAccessException`. A field whose access cannot be established is
now treated as absent during selection, so the normal missing-member
handling applies (`MissingPropertyException` / `MissingFieldException`, as
in Groovy 4) and the indy selectors degrade to the generic `MetaProperty`
or adapter path. Fields whose access _can_ be forced (open modules,
class-path classes, `--add-opens`) are unaffected, and the check is
deliberately not part of member selection, so a call site with its own
access rights can still reach such a field. `CachedField` also caches its
deep-reflection handles and drops the synchronization around them
(https://issues.apache.org/jira/browse/GROOVY-12314[GROOVY-12314]).
* *Compiler-referenced ABI marker*: the new
`@org.apache.groovy.lang.annotation.GroovyABI` annotation marks the
internals that compiled Groovy bytecode links against directly -- call
sites, `ScriptBytecodeAdapter`, indy bootstraps, transform helpers. It
does not make an element public API; it records that compiled Groovy may
outlive the compiler version that produced it, so removing or changing a
marked element is a break for already-compiled code. A mandatory `since`
gives the release in which the element joined the ABI, and `CLASS`
retention keeps the marker in published jars so the build's binary
compatibility tooling can gate the annotated subset
(https://issues.apache.org/jira/browse/GROOVY-12256[GROOVY-12256]).
* `ObjectUtil` is restored as a binary-compatibility facade. It was
removed in Groovy 5, which broke Groovy 4-compiled `@Immutable` classes at
runtime; such classes now load and run again
(https://issues.apache.org/jira/browse/GROOVY-12257[GROOVY-12257]).
...
* AST nodes built by the parser carry accurate source positions
(`lineNumber` / `columnNumber` / `lastLineNumber` / `lastColumnNumber`;
1-based, with an exclusive end column), including annotations, and a
synthetic child no longer overwrites an authored parent's end position.
`-1` is now reserved for genuinely synthetic or generated nodes -- a
`ModuleNode`, a script's wrapping `BlockStatement`, the implicit
`return null`, `EmptyExpression.INSTANCE` / `EmptyStatement.INSTANCE`,
try-with-resources desugaring, `@Trait` and `<clinit>` -- so tooling can
tell authored code from generated code by `lineNumber > 0`, which is what
`SecureASTCustomizer.isFromSource` does
(https://issues.apache.org/jira/browse/GROOVY-12401[GROOVY-12401]).

[[g6-other-module-changes]]
...
* *groovy-json*: `JsonSlurper` also bounds the *length of a number token*
(default 1000 characters, matching Jackson's `StreamReadConstraints`, so
JSON is bounded consistently with the Jackson-backed YAML/TOML/CSV
slurpers). Conversion of a long digit run through `BigInteger`/`BigDecimal`
is superlinear, so an unbounded token made a few hundred KB of input cost
seconds of CPU. Configure it per instance with `setMaxNumberLength(int)` or
globally with `groovy.json.maxNumberLength`; a value `<= 0` disables the
check. See <<Groovy6.0-breaking,Breaking changes>>
(https://issues.apache.org/jira/browse/GROOVY-12329[GROOVY-12329]).
* *groovy-json*: `JsonSlurper.setDateHandling(JsonDateHandling)` says
_which_ type a date-like string becomes, rather than only whether to
convert one. The four choices are `STRING`, `UTIL_DATE` (the default, and
what a slurper has always produced), `INSTANT` and `OFFSET_DATE_TIME` --
the only one that keeps the offset the document carried, since a `Date`
and an `Instant` are both points on the timeline. As before, this applies
to the `INDEX_OVERLAY` and `LAX` parser types; the other two return the
string whichever handling is chosen, and a date carrying no time
(`"2026-09-04"`) is left as a `String` by all four. The boolean
`setCheckDates`/`isCheckDates` pair remains, mapping onto `UTIL_DATE` and
`STRING`, but is deprecated and no longer has any effect once
`setDateHandling` has been called -- in either order, so a choice made
through the wider option is not undone by the narrower one
(https://issues.apache.org/jira/browse/GROOVY-12352[GROOVY-12352]).
* *groovy-xml*: `XmlParser` and `XmlSlurper` now bound element *nesting
depth* at 1000 by default, matching `groovy-json`. Secure processing does
not bound depth, and the JAXP limit that does
(`jdk.xml.maxElementDepth`) defaults to unlimited. The SAX parse itself
survives deep input -- the damage lands on the first consumer to walk the
result recursively (`Node.text()`, `XmlNodePrinter`,
`GPathResult.toString()`, `XmlUtil.serialize`), each of which runs a stack
frame per level, so a 350KB document 50,000 elements deep parsed cleanly
and then killed all of them with a `StackOverflowError` -- an `Error`, and
so outside the `catch (Exception)` an application would reasonably use for
a malformed document. The JDK now enforces the bound during the parse and
names the offending element. Nothing is applied when
`jdk.xml.maxElementDepth` has already been set explicitly
(https://issues.apache.org/jira/browse/GROOVY-12331[GROOVY-12331]).
* *groovy-json*: `JsonOutput.prettyPrint` gains an overload taking the
nesting bound, and bounds the depth it will format by default
(https://issues.apache.org/jira/browse/GROOVY-12330[GROOVY-12330]).
* `ConfigObject.writeTo` now writes keys and values as *data* rather than
as source. `ConfigSlurper.parse` compiles what `writeTo` produced, but
keys were written bare unless they were Groovy keywords and values were
rendered by `FormatHelper.inspect`, which quotes a `String` but not other
types. The documented round trip therefore failed for keys containing
spaces or quotes, for nested blocks under such keys, for `GString`,
`StringBuilder` and most non-`String` values -- and a key holding a
statement was *executed* on re-parse, so an application persisting a
configuration with an attacker-influenced entry name would run it. Every
key is now rendered as an identifier when it is one and as a quoted
literal otherwise, and values are written as data
(https://issues.apache.org/jira/browse/GROOVY-12273[GROOVY-12273]).
...
* *groovy-sql*: `Sql`'s prepared-statement cache is now bounded and
thread-safe. It was an unbounded `HashMap` keyed on the SQL text and mutated
without synchronization, so it grew without limit whenever the text varied
rather than the parameters -- `inList` expands to a different placeholder
count per list size, so a caller sizing the list minted a distinct cached
statement each time, held open until the `Sql` was closed -- and a shared
`Sql` used from more than one thread could corrupt the map. It is now a
synchronized, access-ordered cache that evicts the least recently used
statement past a cap and closes it on the way out, so bounding it does not
leak the cursor it drops. The cap is `statementCacheSize`, default 256, also
settable with `groovy.sql.statement.cache.size`; `0` or less keeps the old
unbounded behaviour. Statement creation stays outside the lock, so preparing
one does not serialize other callers. Where a single `Sql` really is shared
across threads, size the cache above the working set: a bound below it can
now evict and close a statement another thread was about to reuse
(https://issues.apache.org/jira/browse/GROOVY-12371[GROOVY-12371]).
* *groovy-servlet*: gapi:groovy.servlet.GroovyServlet[GroovyServlet] no longer sends error detail to the
client. A failing groovlet put the script path, the exception message and the
top stack frame in the 500 response body -- the same detail already written
to the servlet and application logs -- which discloses internals to callers
and, since the response is `text/html` carrying a request-derived script
path, is a reflected-content risk where the container does not escape the
`sendError` message. The client now gets a generic 500 and the full detail
still goes to the logs. A protected `isVerboseErrors()` hook, defaulting to
the `groovy.servlet.verbose.errors` system property, restores the detailed
responses for debugging
(https://issues.apache.org/jira/browse/GROOVY-12373[GROOVY-12373]).
* *groovy-swing*: gapi:groovy.swing.LookAndFeelHelper[LookAndFeelHelper]'s Metal `theme` attribute treated an
unrecognised string as a class name and resolved, initialised and
instantiated it before discovering whether it was even a `MetalTheme` -- the
cast failed only afterwards -- and resolved through its own class loader, so
an application- or `@Grab`-supplied theme was not found at all. The class is
now resolved without initialising it, through the thread context class
loader, and confirmed to be a `MetalTheme` before construction;
anything else raises `IllegalArgumentException` without being initialised
(https://issues.apache.org/jira/browse/GROOVY-12376[GROOVY-12376]).
...
* *The command-line launchers no longer add the current directory to a
classpath you supplied.* `groovy`, `groovyc`, `groovysh` and friends appended
`.` to whatever came from `-cp`/`-classpath` or the `CLASSPATH` environment
variable, which the `java` launcher does not do. An explicit classpath is now
honoured as-is; only a bare invocation with no classpath given falls back to
the current directory. If you relied on the implicit `.` -- running a script
that loads a class from the working directory while also passing `-cp` for a
library jar -- add `.` to the classpath yourself, e.g. `-cp lib.jar:.`
(`-cp lib.jar;.` on Windows)
(https://issues.apache.org/jira/browse/GROOVY-12374[GROOVY-12374]).
* *String-to-`Class` coercion no longer runs the class's static
initializer.* `'com.example.Foo' as Class`, and the `(Class)` cast the
compiler routes through `ShortTypeHandling.castToClass`, now *resolve* the
named class without *initializing* it: `Class.forName(name, false, loader)`
in place of the single-argument `Class.forName(name)`, with the same class
loader as before. The class is still initialized lazily on first real use, as
the JVM always does. Code that relied on the coercion for the side effect --
most commonly the legacy JDBC idiom `'com.mysql.jdbc.Driver' as Class` to
register a driver -- should call `Class.forName(name)` explicitly; modern
JDBC drivers auto-register through `ServiceLoader` and need none of it. The
signature is unchanged, so this is a behavioural change only
(https://issues.apache.org/jira/browse/GROOVY-12375[GROOVY-12375]).
* *`decodeBase64` rejects malformed padding.* The decoder set a done flag on
`=` but never checked that the padding filled its group, so it accepted a
single `=` where two were needed (`YQ=`), an extra `=` past a full group
(`YQ===`), and padding on an already complete group (`YWJj=`) -- each
decoding as if the padding were correct, so distinct malformed strings mapped
to the same bytes. Padding is now valid only when it fills its
four-character group exactly and the group already holds at least two data
characters; any other shape throws. Deliberately unchanged: *missing*
padding still decodes (`YQ` and `YWJj` both work, as with
`java.util.Base64`), whitespace and the newlines of a chunked encoding are
still ignored so `encodeBase64(chunked: true)` round-trips, and
non-canonical trailing bits are still accepted
(https://issues.apache.org/jira/browse/GROOVY-12372[GROOVY-12372]).
* `GroovyServlet` no longer includes the script path, exception message and
top stack frame in the 500 response body; the client gets a generic error and
the detail goes to the logs. Set `-Dgroovy.servlet.verbose.errors=true` (or
override `isVerboseErrors()`) to restore the detailed responses for
debugging. See <<g6-other-module-changes,Other Module Changes>>
(https://issues.apache.org/jira/browse/GROOVY-12373[GROOVY-12373]).
...
default methods so existing visitors keep compiling. Only a switch whose
value is used is an expression: see <<switch-alignment,Switch: Position
Decides, Not the Arrow>>
(https://issues.apache.org/jira/browse/GROOVY-12255[GROOVY-12255]).
* A `switch` whose value is not used is a switch _statement_, whatever
its arms look like. In Groovy 6.0.0-RC-1 and RC-2 a `switch` with `+->+`
arms was compiled as a switch expression wherever it appeared, so an
unmatched selector threw `IllegalStateException` and, under
`@TypeChecked` / `@CompileStatic`, a missing `default` was a compile
error even where the value was discarded. Position now decides, as in
Java: in statement position an unmatched selector does nothing and no
`default` is required; in expression position the strict rules are
unchanged. A `switch` that ends a method, closure or script body is a
statement whose matched arm value is returned by Groovy's implicit
return, with `null` -- or the primitive default -- when nothing matched,
which is what 4.x and 5.x did. AST transforms see a `SwitchStatement` in
that position. See <<switch-alignment>>
(https://issues.apache.org/jira/browse/GROOVY-12399[GROOVY-12399],
https://issues.apache.org/jira/browse/GROOVY-12408[GROOVY-12408]).
* A duplicate constant `case` label in a switch _statement_ is now a
compile-time error under `@TypeChecked` / `@CompileStatic`, as it already
was in a switch expression and as it is in Java. Only statically known
labels are compared -- int-family, `String` and enum constants. Dynamic
Groovy is unchanged and still takes the first matching arm
(https://issues.apache.org/jira/browse/GROOVY-12406[GROOVY-12406]).
...
(https://issues.apache.org/jira/browse/GROOVY-12191[GROOVY-12191]).
* Generic types must now be *well-formed* by the JLS rules Java applies,
in every compilation unit -- dynamic Groovy and scripts included (see
<<java-generics>>). Constructs that Groovy 4 and 5 accepted by silently
falling back to the erasure are compile errors: generic array creation
(`new T[n]`, `new List<String>[n]`), a generic class extending
`Throwable`, a wildcard in a class instance creation or superinterface, a
primitive type argument (`List<int>`), instantiating or naming a type
variable (`new T()`, `T.class`), a parameterized class literal
(`Foo<String>.class`), a non-first class bound in `<T extends A & B>`, a
parameterized non-static member of a raw type, and catching a type
parameter. `new List<?>[n]` remains legal -- an unbounded wildcard is
reifiable. Two placeholders named `T` from different declarations are now
distinguished, so a method's `T` can no longer be stored in a field typed
by the class's `T`, and duplicate names in one type-parameter section are
rejected. The _Generics_ section of the object orientation guide tabulates
each rejected construct with its workaround
(https://issues.apache.org/jira/browse/GROOVY-12319[GROOVY-12319]).
* Field-backed property access is now consistent across compilation
modes: a field whose access cannot be established -- a non-public field of
a strongly encapsulated class, with no `--add-opens` -- is treated as
absent during member selection rather than being selected and then failing
on use. Such an access now raises `MissingPropertyException` /
`MissingFieldException` (as in Groovy 4) where the `invokedynamic`
property path previously escalated to `GroovyBugError` and attribute
access leaked a raw `IllegalAccessException`. Fields whose access can be
forced are unaffected
(https://issues.apache.org/jira/browse/GROOVY-12314[GROOVY-12314]).
* *groovy-json*: `JsonSlurper` rejects a number token longer than 1000
characters with a `JsonException`, where any length previously parsed.
Raise the bound with `setMaxNumberLength(int)` or
`-Dgroovy.json.maxNumberLength=<n>`, or disable the check with a value
`<= 0`, if you legitimately carry such values
(https://issues.apache.org/jira/browse/GROOVY-12329[GROOVY-12329]).
* *groovy-json*: date handling on `JsonSlurper` moves from a boolean to
the `JsonDateHandling` enum (see <<g6-other-module-changes,Other Module
Changes>>). The default is unchanged -- `UTIL_DATE`, a `java.util.Date`,
as before -- but `setCheckDates`/`isCheckDates` are deprecated, and once
`setDateHandling` has been called on a slurper the boolean has no further
effect on it. In the internal `org.apache.groovy.json.internal` package,
the four-argument `JsonParserLax` and `JsonFastParser` constructors now
take a `JsonDateHandling` in place of the `boolean`; the deprecated
`boolean` form of the `CharSequenceValue` constructor is retained
(https://issues.apache.org/jira/browse/GROOVY-12352[GROOVY-12352]).
* *groovy-xml*: `StreamingSAXBuilder` now applies the comment and
processing-instruction validators its sibling builders already used, so a
comment body containing `--` or ending in `-`, or a processing-instruction
target or data containing `?>`, is rejected rather than passed to the
`ContentHandler`. Such input was never well-formed XML -- the
specification forbids all three -- but it was previously accepted, and
because neither context supports escaping it closed the construct early
and emitted whatever followed as markup
(https://issues.apache.org/jira/browse/GROOVY-12338[GROOVY-12338]).
* *groovy-xml*: `XmlParser` and `XmlSlurper` bound element nesting depth
at 1000 by default, so a document deeper than that is now refused by the
parse that reads it. Set `jdk.xml.maxElementDepth` explicitly to choose a
different bound (or `0` for unlimited) -- Groovy applies nothing when that
property is already set
(https://issues.apache.org/jira/browse/GROOVY-12331[GROOVY-12331]).
* `ConfigObject.writeTo` now emits keys and values as data rather than as
source, so the output of a round trip through `ConfigSlurper.parse`
differs for keys that are not plain identifiers and for values that are
not `String`. Code that parsed `writeTo` output with something other than
`ConfigSlurper`, or that relied on a key being written bare, needs
adjusting; the round trip itself now holds where it previously failed or,
for a key holding a statement, executed
(https://issues.apache.org/jira/browse/GROOVY-12273[GROOVY-12273]).
* *groovy-typecheckers*: `CheckingVisitor.findTargetVariable` is a
`protected` instance method, where 5.x had it implicitly public and
`static`. A third-party checker compiled against 5.x fails with
`NoSuchMethodError`, since the call site was an `invokestatic`, and
source that calls it qualified -- `CheckingVisitor.findTargetVariable(...)`
-- no longer compiles. A subclass calling it unqualified, which is how
the checkers Groovy ships use it, is unaffected. Static and instance
methods cannot share a signature, so no compatibility shim is possible:
recompile against 6.x, and inline the lookup in a caller that is not a
subclass. `groovy-typecheckers` was incubating in 5.x, so this is
migration guidance rather than a broken guarantee
(https://issues.apache.org/jira/browse/GROOVY-12237[GROOVY-12237]).
...
| `groovy.dgm.factory`
| `true`
| `false` makes default Groovy method adapters load by class name
(pre-6 behaviour) instead of through the generated factory. Only
meaningful when passed to `native-image` at build time, where it prunes
adapters an agent recording did not see (see <<native-image>>).
| https://issues.apache.org/jira/browse/GROOVY-12365[GROOVY-12365]

| `groovy.indy.cold.reflection`
| unset
| Reflective cold dispatch tier for plain method calls. Unset: used only
by AOT-linked call sites (native images). `true` enables it on a JVM,
at the cost of loggers and stack walkers seeing runtime frames while a
call site is cold; `false` disables it everywhere. 6.0.0-RC-1 defaulted
to `true`.
| https://issues.apache.org/jira/browse/GROOVY-12137[GROOVY-12137],
https://issues.apache.org/jira/browse/GROOVY-12354[GROOVY-12354]
...
| `1000`
| Invocation count after which `CachedMethod.invoke` installs a generated
direct-invoker trampoline in place of `Method.invoke` (MOP path only).
| https://issues.apache.org/jira/browse/GROOVY-12325[GROOVY-12325]

| `groovy.cachedmethod.invoker.disable`
| `false`
| `true` keeps `CachedMethod.invoke` on `Method.invoke`, never generating a
trampoline.
| https://issues.apache.org/jira/browse/GROOVY-12325[GROOVY-12325]

| `groovy.hidden.classes.disable`
| `false`
| `true` defines Groovy's runtime-generated helper classes through the
classic `defineClass` path instead of JEP 371 hidden classes.
| https://issues.apache.org/jira/browse/GROOVY-12223[GROOVY-12223]

| `groovy.antlr4.cache.size`
| `20000`
| Ceiling on DFA states retained across the shared parser ATN; exceeding it
drops the cache. A value `<= 0` removes the ceiling.
| https://issues.apache.org/jira/browse/GROOVY-12318[GROOVY-12318]

| `groovy.antlr4.cache.threshold`
| `0`
| Parse count after which the shared parser DFA cache is cleared (opt-in;
`0` leaves the size ceiling and the GC canary to do the work). A negative
value switches off both the counter and the canary.
| https://issues.apache.org/jira/browse/GROOVY-12316[GROOVY-12316],
https://issues.apache.org/jira/browse/GROOVY-12318[GROOVY-12318]
...
| `groovy.json.maxNumberLength`
| `1000`
| Maximum length in characters of a JSON number token for `JsonSlurper`;
...
| `groovy.sql.statement.cache.size`
| `256`
| Maximum prepared statements `Sql` keeps in its cache before evicting and
closing the least recently used one; `0` or less restores the pre-6
unbounded cache. Also settable per instance as `statementCacheSize`.
| https://issues.apache.org/jira/browse/GROOVY-12371[GROOVY-12371]

| `groovy.servlet.verbose.errors`
| `false`
| `true` puts the script path, exception message and top stack frame back in
a groovlet's 500 response body (a debugging aid; the detail always goes to
the logs).
| https://issues.apache.org/jira/browse/GROOVY-12373[GROOVY-12373]
|===
